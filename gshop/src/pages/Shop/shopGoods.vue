      ...
      <div class="menu-wrapper" ref="menuWrapper">
        <ul>
          <li class="menu-item" :class="{current : index === currentIndex}" v-for="(good,index) in goods" :key="index" @click="clickMenuItem(index)">
						...
						<img class="icon" :src="good.icon" v-if="good.icon" :alt="good.name">
              {{ good.name }}
            </span>
          </li>
        </ul>
...
export default {
  components: {CardControl,Food,ShopCart},
  name: 'shopGoods',
  data() {
    return {
      scrollY: 0, // 右侧滑动的Y轴实时变化坐标
      tops: [], // 右侧所有分类起始Y坐标:li,列表显示后固定
      food: {}, // 需要显示的food
    }
  },
  mounted () {
  ...
  methods: {
    ...
    _initScroll() {
      ...
      this.foodsScroll = new BScroll('.foods-wrapper', {
        probeType: 3,  // 实时派发scroll事件 默认0不派发 因为惯性滑动不会触发
        click: true
      })
